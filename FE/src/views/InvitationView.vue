...
const email = ref(''); const farmId = ref(''); const role = ref('viewer'); const farms = ref([])
...
async function checkEmail() {
  ...
  loading.value = true
  try {
    const { data } = await api(`/invitations/check-email?email=${encodeURIComponent(email.value)}&farmId=${farmId.value}`)
...
async function sendInvite() {
  ...
  sending.value = true
  try { await api('/invitations', { method: 'POST', body: JSON.stringify({ email: email.value, farmId: farmId.value, role: role.value }) }); checkMessage.value = 'Đã gửi email mời.'; checkColor.value = 'success'; email.value = ''; emailAvailable.value = false } catch (error) { checkMessage.value = error.message; checkColor.value = 'danger' } finally { sending.value = false }
}
</script>
...
<template>
  <main class="auth-page"><va-card class="auth-card"><va-card-title>Mời thành viên vào trại</va-card-title><va-card-content>
    ...
    <form @submit.prevent="sendInvite">
      <va-select v-model="farmId" label="Trại" :options="farms" text-by="name" value-by="id" class="form-field" required @update:model-value="checkEmail" />
